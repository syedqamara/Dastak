...
-(void) registerSchemes{
    ...
    router[VIP_KEY_SCHEME] = ^(DPLDeepLink *link) {
        [weakSelf vipKeysApply:link];
    };
}

-(void)vipKeysApply:(DPLDeepLink *)link {
    DDVIPKeyRequestM *key = [link.queryParameters decodeTo:DDVIPKeyRequestM.class];
    DDUserManager.shared.appliedKey = key;
    if (DDUserManager.shared.isLoggedIn || key.isKeyFromBranch) {
        if (DDUserManager.shared.isLoggedIn) {
            [DDAuthUIManager applyKeys];
        }
    }
}
