...
static const unsigned componentFlags = (NSCalendarUnitYear| NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday |  NSCalendarUnitWeekday);
...
-(NSDate *)copyDateWithFormate:(NSString *)dateFormate {
    NSString *date = [self stringWithFormat:dateFormate];
    return [date dateWithFormate:dateFormate];
}
- (BOOL) isEqualToDateIgnoringTime: (NSDate *) aDate
{
    NSDate *currentObjectDate = [self copyDateWithFormate:@"yyyy-MM-dd"];
    NSDate *comparingObjectDate = [aDate copyDateWithFormate:@"yyyy-MM-dd"];
    return [currentObjectDate isEqualToDate:comparingObjectDate];
...
-(BOOL)isInRangeOfDatesStartDate:(NSDate *)date1 andEndDate:(NSDate *)date2 {
    if ([self isEqualToDateIgnoringTime:date1] || [self isEqualToDateIgnoringTime:date2]) {
        return YES;
    }
    if ([self isEarlierThanDate:date2] && [self isLaterThanDate:date1]) {
        return YES;
    }
    return NO;
}
...
//    if (components1.week != components2.week) return NO;
    ...
    //if need to check that same week dates or not
...
+(NSNumber *) getTrDates:(NSString *) dateString format:(NSString *) format {
    ...
    if ([dateFormatter dateFromString:dateString]){
        return [NSNumber numberWithDouble:[[dateFormatter dateFromString:dateString] timeIntervalSince1970]];
    } else {
        return nil;
    }
}
+(BOOL)isValidCheckInDate:(NSDate *)checkInDate andCheckOutDate:(NSDate *)checkOutDat {
    ...
    if (checkInDate == nil || checkOutDat == nil) {
        return false;
    }
    
    if ([checkInDate isEqualToDateIgnoringTime:checkOutDat]){
        return false;
    }
    
    if ([checkInDate isEqualToDateIgnoringTime:date] && [checkOutDat isInFuture]){
        return true;
    }
    
    BOOL isValid = !([checkInDate isEarlierThanDate:date] || [checkOutDat isEarlierThanDate:date]);
    return isValid;
}
@end
