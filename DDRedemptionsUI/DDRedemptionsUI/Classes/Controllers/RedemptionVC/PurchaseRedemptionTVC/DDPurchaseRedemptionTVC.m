...
@implementation DDPurchaseRedemptionTVC
...
-(void)designUI {
    ...
    [self.purchaseButton setTitleColor:DDRedemptionsThemeManager.shared.selected_theme.text_white.colorValue forState:(UIControlStateNormal)];
    self.purchaseButton.backgroundColor = DDRedemptionsThemeManager.shared.selected_theme.app_theme.colorValue;
    self.purchaseButtonContainerView.backgroundColor = [UIColor clearColor];
    
    self.titleLabel.font = [UIFont DDRegularFont:15];
    self.subTitleLabel.font = [UIFont DDBoldFont:34];
    self.purchaseButton.titleLabel.font = [UIFont DDMediumFont:15];
}
...
- (void)didActionCompleted{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didRedemptionActionPerformed::)]) {
        [self.delegate didRedemptionActionPerformed:nil:DDRedemptionTypePurchase];
    }
}
