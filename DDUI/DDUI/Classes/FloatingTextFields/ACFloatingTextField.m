...
- (CGRect)textRectForBounds:(CGRect)bounds {
    if ([UIApplication isRightToLeftDeviceSettings]){
        return CGRectMake(0, 4, bounds.size.width-4, bounds.size.height);
    }else {
...
-(void)initialization{
    ...
    if (_selectedPlaceHolderColor==nil) {
        _selectedPlaceHolderColor = [UIColor colorWithRed:199/256.0 green:199/256.0 blue:199/256.0 alpha:1.0];
    }
    ...
    if (_selectedLineColor==nil) {
        _selectedLineColor = [UIColor colorWithRed:199/256.0 green:199/256.0 blue:199/256.0 alpha:1.0];
    }
...
-(void)addPlaceholderLabel{
    ...
    _labelPlaceholder.font = [self.font fontWithSize:kPlaceholderFontSize];//[UIFont fontWithName:self.font.fontName size:kPlaceholderFontSize];
...
-(void)showErrorPlaceHolder{
    ...
    if (self.errorText != nil && ![self.errorText isEqualToString:@""]) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            bottomLineView.backgroundColor = weakSelf.errorLineColor;
            [self layoutIfNeeded];
        }];
...
//        [self setValue:self.selectedPlaceHolderColor forKeyPath:@"_placeholderLabel.textColor"];
        Ivar ivar =  class_getInstanceVariable([UITextField class], "_placeholderLabel");
        UILabel *placeholderLabel = object_getIvar(self, ivar);
        placeholderLabel.textColor = self.selectedPlaceHolderColor;
...
//    [self setValue:self.placeHolderColor forKeyPath:@"_placeholderLabel.textColor"];
    ...
    [UIView animateWithDuration:0.3 animations:^{
        weakSelf.labelPlaceholder.font = self.font;
        weakSelf.labelPlaceholder.textColor = weakSelf.placeHolderColor;
        [self layoutIfNeeded];
    } completion:^(BOOL finished) {
        self.labelPlaceholder.hidden = YES;
