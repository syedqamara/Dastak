...
-(void)layoutSubviews {
    ...
    self.buttonWidth = CGRectGetWidth(self.bounds)/3;
    self.labelWidth = self.buttonWidth;
    
    CGRect leftFrame = CGRectMake(self.buttonWidth, 0, 1, self.frame.size.height);
    CGRect rightFrame = CGRectMake(self.buttonWidth * 2, 0, 1, self.frame.size.height);
    
    if (self.leftLine == nil) {
        self.leftLine = [UIView.alloc initWithFrame:leftFrame];
        self.rightLine = [UIView.alloc initWithFrame:rightFrame];
        [self addSubview:self.leftLine];
        [self addSubview:self.rightLine];
    }
    self.leftLine.frame = leftFrame;
    self.rightLine.frame = rightFrame;
    self.leftLine.backgroundColor = self.lineColor;
    self.rightLine.backgroundColor = self.lineColor;
